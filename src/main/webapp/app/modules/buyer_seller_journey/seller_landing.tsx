...
import SellerLeftPanelComponent from './seller_leftpanel';
import 'leaflet';
...
class SellerLanding extends React.Component<MapProps, State> {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.latlng !== prevState.latlng) this.loadMarkersAroundMe();
  }
  ...
  private loadMarkersAroundMe() {
    axios
      .get(`${config.getSupplierGetAroundMeUri}?distance=300&page=0&size=1000&units=km&x=${position[0]}&y=${position[1]}`)
      .then(({ data }) => {
        this.setState({
    ...
    axios
      .get(`${config.getReceiversAroundMeUri}?distance=300&page=0&size=1000&units=km&x=${position[0]}&y=${position[1]}`)
      .then(({ data }) => {
        this.setState({
...
export default connect(mapStateToProps)(SellerLanding);
