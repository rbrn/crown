...
import { getSearchEntities, getEntities, reset } from './resource.reducer';
...
export const Resource = (props: IResourceProps) => {
  ...
  const { resourceList, match, loading } = props;
  return (
    <div>
      <h2 id="resource-heading">
        <Translate contentKey="crownApp.resource.home.title">Resources</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="crownApp.resource.home.createLabel">Create new Resource</Translate>
        </Link>
      </h2>
      <Row>
        <Col sm="12">
            ...
            <AvGroup>
              <InputGroup>
                <AvInput
                  ...
                  placeholder={translate('crownApp.resource.home.search')}
                />
          ...
          {resourceList && resourceList.length > 0 ? (
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('name')}>
                    <Translate contentKey="crownApp.resource.name">Name</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('notes')}>
                    <Translate contentKey="crownApp.resource.notes">Notes</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
              ...
              <tbody>
                {resourceList.map((resource, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>
                      {resource.supplyPointResource ? (
                        <Link to={`supply-point-resource/${resource.supplyPointResource.id}`}>{resource.supplyPointResource.id}</Link>
                      ) : (
                        ''
                      )}
                    </td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="crownApp.resource.home.notFound">No Resources found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
